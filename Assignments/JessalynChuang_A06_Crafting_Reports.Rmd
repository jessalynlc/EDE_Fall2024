...
summary_PM25 <- epa_data %>% 
  drop_na(PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_PM2.5 = round(mean(PM2.5), 2)) %>% #use of round to round values to two decimal places
  arrange(mean_PM2.5)
...
summary_Ozone <- epa_data %>% 
  ...
  group_by(COUNTY) %>% 
  summarize(
    mean_Ozone = round(mean(Ozone),2)) %>% #use of round to round values to two decimal places
  arrange(mean_Ozone)
...
```{r, echo=FALSE, fig.align="left", fig.cap="2018 monthly Ozone levels", fig.height=3.5, message=FALSE, warning=FALSE}
...
#Plotting 2018 ozone concentrations by month
ggplot(ozone_2018, aes(x = Month, y = Ozone))+
  geom_boxplot()+
  labs(title = "Monthly Ozone concentrations (2018)",
      y = expression(Concentration ~ (mu * g / m^3))) #use of expression to have units show up nicely
```

```{r, echo=FALSE, fig.align="left", fig.cap="2019 monthly Ozone levels", fig.height=3.5, message=FALSE, warning=FALSE}
...
ggplot(ozone_2019, aes(x = Month, y = Ozone))+
  geom_boxplot()+
...
https://github.com/jessalynlc/EDE_Fall2024

...
O3.density <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  labs(x = "Ozone_AQI_value", y = "Density", fill = "")
...
EPAair_Corr <- cor(EPAair_data)
corrplot(EPAair_Corr, upper = "ellipse")
...
model <- lm(data = EPAair, PM2.5 ~ Ozone + Month + mean_Lng + mean_Lat)
summary(model)

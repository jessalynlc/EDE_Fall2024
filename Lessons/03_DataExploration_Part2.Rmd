...
>Answer: We have to understand what the data set contains, and clean it up.
...
#Chose bar plot because discharge.mean.approval is categorical data
...
#scale_x_continuous says i just want values up to 500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
#Looking at a different value here
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#Why is mean higher than max and min? Because these are counts and not actual values.
ggplot(USGS.flow.data) +
...
#above we specified the color outside of aes, here color is inside aes. Means we are going to apply some attribute of our plot based on values in our data frame here.
ggplot(USGS.flow.data) +
...
#Similar to box plots, but it gives a bit more info because width is proportional to the number of values in there
ggplot(USGS.flow.data) +
...
> Answer: scatter plot helpful for looking relationship between two continuous variables

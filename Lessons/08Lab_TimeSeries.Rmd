...
#Create a sequence, do inner join/left join with inner sequence with your data frame and it will make your NAs show up (so on assignment, we'll do it a different way)
...
ggplot(BR_complete, aes(x = Date, y = Rainfall)) + 
  geom_line() + labs(title = "Rainfall") + geom_smooth(method = 'lm', se = FALSE)
#There is seasonality to this dataset

ggplot(BR_complete, aes(x = Date, y = Temperature_C)) + 
  geom_line() + labs(title = "Temperature [C]") + geom_smooth(method = 'lm', se = FALSE)
...
Rainfall_decomposed <- stl(BR_Rainfall_ts,s.window = "periodic")
plot(Rainfall_decomposed)
#bars of right give you magnitude of component for the dataset. Representative of how much you're "zooming" in to view the trend. The seasonal component here is more important than the trend component since it has a shorter bar. Larger bar means the less strength the component will have.

Temperature_decomposed <- stl(BR_Temperature_ts, s.window = "periodic")
...
Rainfall_components <- mutate(Rainfall_components, Observed = BR_complete$Rainfall, Date = BR_complete$Date)

Temperature_components <- as.data.frame(Temperature_decomposed$time.series[,1:3])
Temperature_components <- mutate(Temperature_components, Observed = BR_complete$Temperature_C, Date = BR_complete$Date)




# Exercise 10: Visualize how the trend maps onto the data for both series
ggplot(Rainfall_components) + geom_line(aes(x = Date, y = Observed)) + geom_line(aes(y = trend, x = Date), color = "#c13d75ff")

ggplot(Temperature_components) + geom_line(aes(x = Date, y = Observed)) + geom_line(aes(y = trend, x = Date), color = "#c13d75ff")

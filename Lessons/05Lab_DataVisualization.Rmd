...
surface_temps_day <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(mapping = aes(
      x=daynum,
      y=temperature_C,
...
surface_temps_date <- PeterPaul.chem.nutrients %>% 
  ggplot(mapping = aes(
      x=sampledate,
      y=temperature_C,
      color=depth)

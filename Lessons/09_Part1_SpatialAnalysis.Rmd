...
#x and y axis are geographic x and y, shape follows shape of NC@
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T), #What does "na.rm=T" do? Is it necessary? calculate the mean, ignore records with missing data, just calculate mean with what you have!
...
#mapview will plot it but has no idea where on earth this location is. We as humans can recognize what is longitude and latitude, but sf package does not assume where coordinates are on earth
...
#1 unit here is going to be degrees because we are using the 4326 coordinate system, so distance units are in degrees

#View the result
mapview(the_buffer)
#It's the same dataframe, but it's now a polygon
```
...
#Projected CRS: WGS 84/ UTM zone 17N. All projected crs actually have underlying geographic coordinate ref system. We translate x and ys to a certain origin. Where is origin? Well we have to use a geographic crs.

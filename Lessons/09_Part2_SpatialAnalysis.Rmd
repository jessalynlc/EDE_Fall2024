...
#Create a map
mapView(huc8_sf, col.regions = "gray", alpha.regions = 0.5) +  # Base layer with light blue and some transparency
  mapView(huc8_sf2, col.regions = "yellow", alpha.regions = 0.7, color = "blue", lwd = 2)       # Top layer with red and higher opacity

ggplot() + 
  geom_sf(data = huc8_sf, fill = 'gray') +
  geom_sf(data = huc8_sf2, fill = 'yellow', color = 'red')
...
wind_raw <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", 
  ...
  mutate(huc8_sf = substr(HUC_12, 0, 8)) %>% 
  group_by(huc8_sf) %>% 
...
TLDR: polygons are defined by vertices around them. If x y values aren't in latitude and longitude, to calculate it's gonna be in squared degrees. You have to project your data in order to do measurements. 
...
huc8_intersect <- huc8_sf_utm %>%  
  st_filter(triCo, .predicate = st_intersects)

mapview(huc8_intersect)

#Intersect the HUC_8s
huc8_intersect2 <- triCo %>% st_intersection(huc8_sf_utm)

huc8_intersect2 <- huc8_intersect2 %>% 
  mutate(Area_m2 = as.numeric(st_area(geometry)))

mapview(huc8_intersect2)
```

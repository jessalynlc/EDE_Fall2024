...
NTL.phys.data1990_1999 <- filter(NTL.phys.data, lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: The rown numbers might not be in our control, so we want to filter on the data itself and not how the data is organized
...
NTL.phys.data.ex5 <- pivot_longer(NTL.phys.data, )
